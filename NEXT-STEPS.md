...
5. **Spike F4: Kanban drag-and-drop.** Candidates: `drag_and_drop_lists`, `appflowy_board` (check it compiles on Flutter 3.47 first; `appflowy_editor` did not), `boardview`, or `super_drag_and_drop` (the team has used it) with a hand-rolled column layout. Build a probe page under Diagnostics that renders 200 cards across 5 columns from generated data (no client data), supports cross-column drop with haptics and edge auto-scroll, and reports frame timings from `SchedulerBinding.addTimingsCallback` (target: no jank at 60 fps on the emulator; a physical device run later). Cards use `BoardhopColors` for type and state, columns use the team's WEF column colors when available (DESIGN.md §3). Decide the package, note the finding in research/spikes/results/README.md, keep the winner as a dependency and drop the rest.
