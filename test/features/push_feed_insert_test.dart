...
void main() {
  ...
  group('inserting it', () {
    ...
    test('the next copy of the same artifact replaces it, never doubles it',
        () async {
      await repository.insertPushed(
        org,
        pushedActivityItem(pointer(), account)!,
    ...
    test('does not mark the feed seen: the row is new until it is opened',
        () async {
      await repository.insertPushed(
        org,
        pushedActivityItem(pointer(), account)!,
    ...
    test('announces the key, so the poll does not notify a second time',
        () async {
      final item = pushedActivityItem(pointer(), account)!;
  ...
  group('a tap on a background-posted notification', () {
    ...
    test('an ordinary route payload is left to the caller', () {
      expect(coordinator.handleTapPayload('/a/account-1/orgs/contoso/activity'),
          isFalse);
