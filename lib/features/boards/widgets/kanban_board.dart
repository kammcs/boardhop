...
///
/// [textScale] raises the cap with the user's text size (r2 §6): a card
/// whose title has grown to AX5 crammed into 320 dp is unreadable, so the
/// ceiling follows the scaler up to 1.6x. The floor does not move — a
/// narrower column than 240 helps nobody.
double kanbanColumnWidth(double viewportWidth, {double textScale = 1.0}) =>
    (viewportWidth * 0.82).clamp(240.0, 320.0 * textScale.clamp(1.0, 1.6));

/// The text scale factor the repo measures layouts against: how much bigger
/// 14 dp type is right now.
double boardTextScale(BuildContext context) =>
...
class _KanbanBoardState<T extends Object> extends State<KanbanBoard<T>>
  ...
  void onDragAutoScroll(Offset local, Size size) {
    ...
    DragSession.dragScrollBy(
      _verticalFor(widget.columns[column].id),
      DragSession.dragScrollDelta(local.dy, _headerHeight, size.height),
    );
  }

  void _hover(int column, int slot) => hoverDragSlot((column, slot));
  ...
  Widget build(BuildContext context) {
    _headerHeight = boardHeaderHeight(context);
    return dragListener(
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          // The horizontal safe-area padding is the shell's floating rail
