...
- Tablet navigation: on iOS and macOS the project shell uses `GlassNavigationRail` (a floating, blurred, translucent pill centered on the left, spanning about 80% of the height with the destinations spread evenly along it, over the page background, `Radii.xl` corners); elsewhere the Material `NavigationRail`. Phones keep the bottom `NavigationBar` everywhere.
