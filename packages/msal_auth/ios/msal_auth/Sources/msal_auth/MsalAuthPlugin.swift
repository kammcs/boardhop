...
public class MsalAuthPlugin: NSObject, FlutterPlugin, FlutterSceneLifeCycleDelegate {
    ...
    public func handle(
        ...
        case "createSingleAccountPca", "createMultipleAccountPca":
            ...
            let clientCapabilities = dict["clientCapabilities"] as? [String]
        ...
        case "acquireToken":
            ...
            let claims = dict["claims"] as? String

            acquireToken(
                scopes: scopes, promptType: promptType, loginHint: loginHint, authority: authority,
                customWebViewConfig: customWebViewConfig, claims: claims,
                result: result)
        case "acquireTokenSilent":
            ...
            acquireTokenSilent(
                ...
                claims: claims, forceRefresh: forceRefresh, result: result)
    ...
    private func createPublicClientApplication(
        ...
        if let clientCapabilities, !clientCapabilities.isEmpty {
            pcaConfig.clientApplicationCapabilities = clientCapabilities
        }
    ...
    private func acquireToken(
        ...
        customWebViewConfig: [String: Any]?, claims: String?,
        result: @escaping FlutterResult
    ) {
    ...
    private func acquireTokenSilent(
        ...
        claims: String?, forceRefresh: Bool,
        result: @escaping FlutterResult
    ) {
        guard let pca = MsalAuth.publicClientApplication else {
        ...
        if MsalAuth.pcaType == PublicClientApplicationType.single {
            getCurrentAccount { currentAccount in
                ...
                self.executeSilentRequest(pca: pca, account: currentAccount, scopes: scopes, authority: authority, claims: claims, forceRefresh: forceRefresh, result: result)
            }
            return
        }
        ...
        executeSilentRequest(pca: pca, account: account, scopes: scopes, authority: authority, claims: claims, forceRefresh: forceRefresh, result: result)
    }
