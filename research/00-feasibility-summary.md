...
**Date:** 2026-09-10
**Target:** iOS + Android, phone + tablet, Flutter (decided 2026-09-10; React Native / Expo was the original assumption, see section 0). A client for Azure DevOps Services that blends the Jira mobile app (boards, work items) and the GitHub mobile app (repos, PR review) experiences.
...
| **Tablets at launch: responsive phone layout that scales.** True master-detail panes in v2. | SDK `NavigationRail` and `LayoutBuilder` breakpoints; `flutter_adaptive_scaffold` is discontinued, so no scaffold package. |
...
| **Platforms: iOS and Android together**, TestFlight and Play internal testing before public listing. | One Flutter codebase; Codemagic or GitHub Actions macOS runners for iOS; no first-party OTA, so the release pipeline must be smooth from day one. |
| **Rich text: full rich-text editor in v1** for work item descriptions. | **`html_editor_enhanced` (Summernote in a WebView), decided by spike F3 on 2026-09-10:** it round-tripped a real 13 KB description with tables, colored cells, images and mentions at 100% text recall, while the Delta-based editors (Quill, Fleather) dropped every table and every mention identity across the sample corpus (41% of real fields contain tables). Read views render with `flutter_widget_from_html_core`. A native-feeling Quill editor with opaque embeds for tables and mentions stays a later option. Plus a Markdown editing path. Non-negotiable safeguards: detect the field format before editing (`multilineFieldsFormat`, returned at 7.1 when the read has no `fields` filter; see section 6a), never convert between HTML and Markdown, dry-run with `validateOnly=true`, and use the `test /rev` op so a stale edit fails instead of overwriting. |
| **Enterprise broker support in v1.** Adopt the `msal_auth` Flutter package (native MSAL iOS and Android with the Authenticator broker). | Unblocks tenants with device-compliance Conditional Access from day one. Needs `msauth.{bundleId}://auth` and Android signature-hash redirect URIs. `msal_auth` does not expose claims challenges, so add a fork or platform channel for Azure DevOps Continuous Access Evaluation. Intune, if ever required, is a kammcs-owned Flutter plugin over the native SDKs (document 08 section 4). |
...
| **Offline: read cache plus queued writes.** Comments, votes, state changes and card moves queue and flush on reconnect. | `drift` for the cache and queue. Every queued write carries the `rev` it was made against; a rejected `test /rev` surfaces a conflict screen rather than silently overwriting. |
...
| **Telemetry: crash reports only, scrubbed.** No usage analytics. | `sentry_flutter` or Crashlytics; redact tokens, org names, titles, paths and code from crash payloads. Declare only crash diagnostics in store privacy forms. |
...
### Stack decision: Flutter (locked 2026-09-10)

Before scaffolding, Kelly asked whether Expo is the easiest path or whether Microsoft's own stack or Flutter is better suited. Three evaluations ([08a MAUI](08a-dotnet-maui-evaluation.md), [08b Flutter](08b-flutter-evaluation.md), [08c React Native and others](08c-react-native-and-other-stacks.md)) are compared in [08-stack-comparison.md](08-stack-comparison.md). **Kelly locked in Flutter on 2026-09-10.** A maintained broker-capable MSAL wrapper (`msal_auth`) exists, kammcs has shipped two Flutter apps with most of Boardhop's building blocks, and the v1 native MSAL module for React Native disappears. Costs: no first-party over-the-air updates, iOS text-input fidelity, HTML editing through Delta conversion or a WebView. .NET MAUI only wins if a named prospect requires Intune App Protection Policies. The decision table above now carries the Flutter equivalents. Document 03 (Expo stack) is retained as history; document 08b is the stack reference going forward. React Native and Expo mentions in sections 1 to 7 below predate the switch and are left as written.
...
A note on "standard MSAL login flow": Microsoft ships no supported MSAL for Flutter or React Native. On Flutter, `msal_auth` wraps the current native MSAL SDKs with broker support and is actively maintained, which is the main reason the stack moved from Expo to Flutter. The Entra PKCE flow is the same either way; native MSAL owns token acquisition, silent refresh, and the token cache.
