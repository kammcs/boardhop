...
| M6 | **Token: styled text, edits break it.** `@Kelly Kamm` in `BoardhopColors.mention` (the link blue; Kelly asked for mentions to be very obvious in both themes, so not the slate primary), semibold. Backspace deletes one character; any edit inside the run turns it into plain text (no longer a mention), which is what the server would do with it anyway. The caret may sit inside a token. |
