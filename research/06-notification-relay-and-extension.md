...
- **Built 2026-09-13 (`extension/`, published private 0.1.0):** the hub lives under Organization settings and Project settings; it stores only the relay URL (Extension Data Service, collection scope, `boardhop/settings`) and never the key; the key it takes is the org's hook secret, which also authenticates the hub's calls to the relay's org-key routes (`/v1/orgs/{org}/…`, `relay/README.md`) — so provisioning needs no relay-wide credential in the browser. Subscription health comes from Azure DevOps's own `status` per subscription; Repair re-creates anything not `enabled`. The Extension Data scope is `vso.extension.data_write`; the app-side discovery read (spike 1) is still open.
...
5. ~~Private publish~~ **half done 2026-09-13**: publisher `KammCS` exists, the real extension (`extension/`, not a hello-world) is published private as `KammCS.boardhop` 0.1.0, validated, and shared with `puremedia` and `kammcs` through `tfx`. Still to confirm: the install itself and the hub loading (first real install), and the data-store read from the app with the user's token (spike 1).
