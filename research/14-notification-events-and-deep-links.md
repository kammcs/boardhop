...
| tfs | `git.pullrequest.merged` | 1.0 | none | yes, author only, merge failures only |
...
| pipelines | `ms.vss-pipelines.run-state-changed-event` | 5.1-preview.1 | none | **no** (superseded by `build.complete`; see §1.4) |
...
Payload facts verified by capture (w22/w23, All + text): `workitem.updated`, `workitem.commented` and the PR comment event. **Not yet captured:** `git.pullrequest.created/updated/merged`, `build.complete`, `run-state-changed`, `approval-pending`. Their shapes below come from the Microsoft reference and are marked *(unverified)* where routing depends on them; §7 lists the two spikes that close this before the build.
...
One event covers reviewer votes, reviewer list changes, status changes (active/abandoned/completed/draft) and new pushes (iterations), and the body has no "what changed" field. Two ways to know the kind: diff against relay-kept state (§5) or **subscribe four times with the `notificationType` filter** the publisher exposes for this event (s39). The second is stateless and self-describing, so R2 does that; the relay maps `subscriptionId → kind`. The accepted values of `notificationType` are not in the s39 table (only the input's name is), so spike **s41** (§7) reads the input descriptor's values first; the working assumption from the web UI's filter is `PushNotification`, `ReviewersUpdateNotification`, `StatusUpdateNotification`, `ReviewerVoteNotification`. If the filter turns out not to be usable, fall back to §5 state diffing (reviewer ids + votes, status, `lastMergeSourceCommit.commitId`, all metadata).
...
| `workitem.commented` | always | assignee; creator when different; mentioned users get "mentioned you" instead | commented on #{id} | `wi.{id}.comments` | same +`?comment={resource.comment.id}` *(field name unverified at v1.0; w22 saw the comment text under `resource.comment`)* | open, scroll to Discussion and highlight the comment |
...
| `ms.vss-code.git-pullrequest-comment-event` | comment on a thread | PR author; every other author in the same thread (parent and siblings, from the payload's thread comments when present *(the v2 body carries `comment` and `pullRequest`; whether it carries the whole thread is unverified)*); reviewers who have voted; mentioned users get "mentioned you" | commented on / replied on !{id} (replied when `parentCommentId` > 0) | `pr.{id}.t{threadId}` | same +`?thread={threadId}` | open the PR on Comments, scroll to the thread; a file thread opens the file diff at the thread (the page already does this from the Comments tab) |
| `git.pullrequest.merged` | `mergeStatus` conflicts or failure | author | merge failed on !{id} | `pr.{id}` | same | open the PR overview |
...
| `approval-completed` | `status` approved or rejected | the other approvers of the same approval, and the run's requester | approved / rejected … | `approval.{id}` | `/pipelines/runs/{runId}` | replaces the pending notification (same collapse key), opens the run page |
...
| table | columns | why |
|---|---|---|
| `pr_state` | `org, prId, status, isDraft, sourceCommit, reviewers (json: id → vote), authorId, updatedAt` | added-reviewer detection, "pushed" detection, PR author for PR builds; fallback when `notificationType` is not usable |
...
3. **Two hooks for one action.** A work item comment fires `workitem.commented` **and** `workitem.updated` (with `System.History` and `System.CommentCount` in `fields`, seen in w22). The relay treats a `workitem.updated` whose changed fields are a subset of `{System.History, System.CommentCount, System.Rev, System.ChangedDate, System.ChangedBy, System.AuthorizedDate, System.RevisedDate, System.Watermark, System.AuthorizedAs, System.PersonId}` as **comment noise and drops it**; the `commented` event is the one that notifies. Likewise a vote fires a system comment on the PR comment event; system comments are dropped and the ReviewerVote update notifies.
...
|---|---|---|---|
| **s41** hook input values | read-only | `GET _apis/hooks/publishers/tfs` and read the `notificationType` input descriptor's `values` for `git.pullrequest.updated`; also `changedFields` for work items | nothing; run now |
| **w24** PR capture | scratch write | Re-create the All capture set (w22 `W22_MODE=create W22_RESOURCE_DETAILS=all`), then on the scratch repo: branch from main, push one commit through the app's `pushEdit` path or REST, create PR (not draft) with Kelly as reviewer → `created`; add a second reviewer → ReviewersUpdate; vote from the spike identity → ReviewerVote; push a second commit → Push; set draft/publish, then abandon → StatusUpdate; comment in a thread with an `@<guid>` mention → comment event with mention. Record shapes, delete the capture set, wipe the captures. | Kelly's OK for a new scratch PR (8334 and 8336 stay as they are) |
| **w25** pipeline capture | scratch write, **costs hosted minutes** | Queue one run of `boardhop-scratch` (id 139): captures `run-state-changed`, `approval-pending` (Deploy environment, Kelly grants or rejects from the phone, which is itself the R2 acceptance test), `stage-state-changed`, `approval-completed`, `build.complete` v2 with `requestedFor`/`requestedBy`/`triggerInfo` | Kelly's explicit go-ahead (rule 3: parallelism minutes on the puremedia org), about 3 minutes of hosted agent time |
| **s42** `workitem.commented` v1.0 body | read the w22 capture notes | confirm `resource.comment.id` exists at 1.0 so the `?comment=` anchor is real; if not, subscribe at `5.1-preview.3` | nothing; the capture on the box was wiped, so it is part of w24's run |
...
- **R2.8 hooks provisioning for the beta.** `w22` generalized into the relay's own `tool/hooks.dart` that creates the twelve subscriptions per project against `/hooks/{org}` with a per-org secret, used by hand for the scratch project and by the extension hub later.
