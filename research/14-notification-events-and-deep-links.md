...
Payload facts verified by capture (All + text; w22/w23 on 2026-09-13, then w24/w25 the same day, key trees in `research/spikes/results/w24_r2_capture_shapes.md`, summary in that folder's README): every event in the table except `build.complete` and `approval-completed`, which arrive when Kelly decides the Deploy approval on run 20260913.1. Shapes still marked *(unverified)* below are those two.
...
| `workitem.commented` | always | assignee; creator when different; mentioned users get "mentioned you" instead | commented on #{id} | `wi.{id}.comments` | same +`?comment={resource.commentVersionRef.commentId}` (verified at v1.0, w24) | open, scroll to Discussion and highlight the comment |
...
| **w25** pipeline capture | scratch write, **costs hosted minutes** | **Run 2026-09-13** (`w25_r2_pipeline_run.py`): run 20260913.1 (build 20163) queued; `run-state-changed`, two `stage-state-changed` and `approval-pending` captured (shapes in §1.4). **Waiting on Kelly** to approve or reject Deploy from the phone, which delivers `approval-completed` and `build.complete` v2. | Kelly's decision on the approval |
