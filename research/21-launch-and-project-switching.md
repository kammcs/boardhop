...
| L3 | Old pages | **Keep both routes, out of the main flow.** ~~`/orgs` becomes "Manage accounts" reachable from the picker; the project list stays for deep links and the org row.~~ **Revised 2026-09-16 (Kelly):** nothing in the UI leads to the Accounts page or the project list any more: the picker's organization row is a plain subhead and the Manage accounts row is gone; both pages remain only as routes (deep links, the no-account launch fallback). Only the launch redirect changes. |
